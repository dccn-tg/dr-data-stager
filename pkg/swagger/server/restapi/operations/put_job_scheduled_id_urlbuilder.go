...
func (o *PutJobScheduledIDURL) Build() (*url.URL, error) {
	...
	var _path = "/job/scheduled/{id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on PutJobScheduledIDURL")
	}
...
func (o *PutJobScheduledIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PutJobScheduledIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PutJobScheduledIDURL")
	}
