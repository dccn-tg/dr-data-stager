...
func (o *GetJobsStatusURL) Build() (*url.URL, error) {
	...
	var _path = "/jobs/{status}"

	status := o.Status
	if status != "" {
		_path = strings.Replace(_path, "{status}", status, -1)
	} else {
		return nil, errors.New("status is required on GetJobsStatusURL")
	}
...
func (o *GetJobsStatusURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetJobsStatusURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetJobsStatusURL")
	}
